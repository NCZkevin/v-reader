...
<script>
  ...
  export default {
    ...
    methods: {
      getBookDetail (id) {
        this.axios.get(`/api/book?id=${id}`).then(response => {
...
.book_info .desec .bookname {
  padding-bottom: 12px;
}
.book_info .desec .aut_about {
  height: 72px;
}
.book_info .desec .aut_ul {
  margin: 0;
  padding: 0;
  width: 100%;
  color: #8c8c8c;
...
.book_info .desec .aut_ul .aut_p span {
  width: 50%;
  float: left;
}
.book_info .desec .aut_last {
